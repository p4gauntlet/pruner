...
bit<32> ZqTDGWH() {
    {
        bit<64> sGPTcR = 64w8331771506825847071;
        {
            ...
            sGPTcR = sGPTcR;
            bit<32> LRpMbf = 32w37657 |-| (bit<32>)sGPTcR;
        ...
        noVnDj = 64w859692371;
        sGPTcR = 64w0;
        sGPTcR = (sGPTcR + 64w6629169280337834382 << 8w55) + noVnDj;
    ...
    return 32w839498710 | (bit<32>)AfXLZG;
}
Vnytmj IBaXWsK(bit<64> yQJV, bit<16> gOEI) {
    bit<8> kXmlrB = ((bit<43>)yQJV |+| 43w1462566353592)[16:9];
    {
        kXmlrB = kXmlrB;
        kXmlrB = kXmlrB;
...
control PFizfTj(out bit<16> lcNR, bit<128> SZxu) {
    bool oHvtbH = !(SZxu[78:22] |-| 57w128961855891103457 | 57w83480883236889332 != (bit<57>)lcNR || true);
    bit<64> nYJwrn = 64w12914525900351020762;
    bool htHGwZ = oHvtbH;
    bit<32> xQNzSP = 32w1305108261;
    bit<128> SYsGZI = 128w18672238;
    bool NdjDFo = ((bit<113>)nYJwrn |+| (bit<113>)lcNR)[62:9] != 54w2143205456929516 || true;
    action woqMg(inout bit<4> OcIW, out Vnytmj ILUP, bit<128> YFnT) {
        ...
        OcIW = -OcIW |+| ((bit<78>)ZqTDGWH())[73:70];
    ...
    apply {
        xQNzSP = xQNzSP + ZqTDGWH() |-| 32w2827375294;
        ZqTDGWH();
        SYsGZI = SYsGZI;
        xQNzSP = xQNzSP;
        IBaXWsK(64w9569382937354443357, 16w0);
        SYsGZI = SYsGZI + 128w165438478943825846563807071914571104592 | SZxu;
    }
}
